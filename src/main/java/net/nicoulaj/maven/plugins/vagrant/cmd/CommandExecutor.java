 ...
 */
package net.nicoulaj.maven.plugins.vagrant.cmd;
    ...
    class Factory
    {
        ...
        private static final class DefaultCommandExecutor implements CommandExecutor
        {
                    ...
                    boolean failsOnErrorOutput ) throws ExecutionException
            {
                ...
                try
                {
                    logger.debug( "VAGRANT-040-000: Executing command: Commandline = " + commandline );
                    result = CommandLineUtils.executeCommandLine( commandline, stdOut, stdErr );
                    if ( logger != null )
                    {
                        logger.debug( "VAGRANT-040-000: Executed command: Commandline = " + commandline + ", Result = "
                                + result );
                    }
                    else
                    {
                        System.out.println( "VAGRANT-040-000: Executed command: Commandline = " + commandline
                                + ", Result = " + result );
                    }
                    if ( failsOnErrorOutput && stdErr.hasError() || result != 0 )
                    {
                        throw new ExecutionException( "VAGRANT-040-001: Could not execute: Command = "
                                + commandline.toString() + ", Result = " + result );
                    }
                }
                catch ( CommandLineException e )
                {
                    throw new ExecutionException( "VAGRANT-040-002: Could not execute: Command = "
         ...
         * NOTE: I am using this to decide whether to fail the Maven build. If the compiler implementation chooses to
        ...
        static class ErrorStreamConsumer implements StreamConsumer
        {
            ...
            public ErrorStreamConsumer( Log logger, ErrorListener errorListener, boolean captureStdErr )
            {
                ...
                if ( logger == null )
                {
                    System.out.println( "VAGRANT-040-003: Error Log not set: Will not output error logs" );
                }
                error = false;
            }
